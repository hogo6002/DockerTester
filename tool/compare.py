...
def get_cves_from_osv(parsed_json):
  cve_ids = set()
  all = []
  for pkg in parsed_json['results'][0]['packages']:
...
def main():
  args = sys.argv[1:]
  global ecosystem, path_base
  ecosystem = args[0]
  path_base = f'../result/{ecosystem}'
  ...
  parsed_json = load_json('osv')
  osv_result = get_cves_from_osv(parsed_json)
  ...
  f.write(f'OSV:\nOriginal: {len(osv_result[0])}\nfinal: {len(osv_result[1])}\n{osv_result[1]}\n\n')
